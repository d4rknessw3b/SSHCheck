...
- 🚫 **Авто-блокировка** — опциональная блокировка IP через `ufw` и `iptables` (с полной поддержкой Docker, Marzban и 3x-ui)
...
| `auto_block` | `false` | Автоблокировка через ufw и iptables (совместимо с Docker-панелями) |
